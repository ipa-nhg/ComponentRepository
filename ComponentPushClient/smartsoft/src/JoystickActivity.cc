...
void JoystickActivity::on_JoystickServiceIn(const CommBasicObjects::CommJoystick &input)
{
...
int JoystickActivity::on_execute()
{
	...
	status = this->joystickServiceInGetUpdate(joystickServiceInObject);
	if(status != Smart::SMART_OK) {
		std::cerr << status << std::endl;
		// return 0;
	} else {
		std::cout << "received: " << joystickServiceInObject << std::endl;
	}
