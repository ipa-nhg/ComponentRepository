...
#include "ComponentPushServerSeRoNetSDKPortFactory.hh"
...
static ComponentPushServerSeRoNetSDKPortFactory SeRoNetSDKPortFactory;

ComponentPushServerSeRoNetSDKPortFactory::ComponentPushServerSeRoNetSDKPortFactory()
{  
	componentImpl = 0;
	ComponentPushServer::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentPushServerSeRoNetSDKPortFactory::~ComponentPushServerSeRoNetSDKPortFactory()
{  }

void ComponentPushServerSeRoNetSDKPortFactory::initialize(ComponentPushServer *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentPushServerSeRoNetSDKPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentPushServerSeRoNetSDKPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
Smart::IPushServerPattern<CommBasicObjects::CommJoystick> * ComponentPushServerSeRoNetSDKPortFactory::createJoystickServiceOut(const std::string &serviceName)
{
	return new SeRoNet::OPCUA::Server::PushServer<CommBasicObjects::CommJoystick>(componentImpl, serviceName);
}


int ComponentPushServerSeRoNetSDKPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentPushServerSeRoNetSDKPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentPushServerSeRoNetSDKPortFactory::destroy()
{
